...
class FishMeasureApp:
    def __init__(self, root):
        ...
        tk.Label(frame_params, text="Facteur de perspective :").grid(row=0, column=0, sticky="w")
        self.entry_persp = tk.Entry(frame_params, width=10)
        self.entry_persp.grid(row=0, column=1, sticky="w", padx=5)
        self.entry_persp.insert(0, "1.15")  # valeur par défaut

        # Type de poisson (liste déroulante)
        tk.Label(frame_params, text="Type de poisson :").grid(row=1, column=0, sticky="w", pady=(5, 0))
        ...
        self.fish_combo.grid(row=1, column=1, sticky="w", padx=5, pady=(5, 0))
    ...
    def measure_on_image(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Erreur",
                "Facteur de perspective invalide. Merci de saisir un nombre (ex : 1.15)."
            )
            return
        ...
        fig, ax = plt.subplots()
        ax.imshow(img)
        ax.set_title("1) Clique sur les deux extrémités du LEURRE (12 cm)")
        plt.axis("on")
